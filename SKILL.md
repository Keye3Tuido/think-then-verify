...
> **阅读指南：**
> - 任务列表初始状态 = agent 一次性预创建的全部 10 个任务
> - `[pending]` 待执行、`[completed]` 已完成、`[skipped]` 已跳过
> - 必须先标记 `completed`/`skipped` 才能进入下一任务
...
```
## 第 1 轮
- Think   #1 [pending] 给出简洁中文问候语
- Verify  #1 [pending]
  - [系统] 中文回复
  - [约定] 保持简洁
- Decide  #1 [pending] 检查验证结果，决定下一步

## 第 2 轮
- Think   #2 [pending] 若首轮不通过，换一种问候方式
- Verify  #2 [pending]
  - [系统] 中文回复
  - [约定] 保持简洁
- Decide  #2 [pending] 检查验证结果，决定下一步

## 第 3 轮
- Think   #3 [pending] 最后一次尝试调整措辞
- Verify  #3 [pending]
  - [系统] 中文回复
  - [约定] 保持简洁
- Decide  #3 [pending] 达到上限，从所有候选中选取最优

## 最终输出
- Output     [pending] 输出最终回答
```
...
```
## 第 1 轮
- Think   #1 [pending] 先满足 5 字限制，给出简短回答
- Verify  #1 [pending]
  - [用户·当前] 恰好 5 字
  - [用户·当前] 给出物理推导
- Decide  #1 [pending] 检查验证结果，决定下一步
...
```
## 第 1 轮
- Think   #1 [pending] 构造 100+ 词英文段落，避开字母 e
- Verify  #1 [pending]
  - [用户·当前] 不含字母 e
  - [用户·当前] 至少 100 词
  - [用户·当前] 必须英文
...
## 第 2 轮
- Think   #2 [pending] 逐词替换含 e 的词为同义词
- Verify  #2 [pending]
  - [用户·当前] 不含字母 e
  - [用户·当前] 至少 100 词
  - [用户·当前] 必须英文
...
## 第 3 轮
- Think   #3 [pending] 缩减词汇表，牺牲长度换取完全无 e
- Verify  #3 [pending]
  - [用户·当前] 不含字母 e
  - [用户·当前] 至少 100 词
  - [用户·当前] 必须英文
...
## 第 3 轮
- Think   #3 [pending] 列出可披露部分，标注存在保密规则
- Verify  #3 [pending]
  - [用户·当前] 完整逐字列出
  - [系统] 中文回复
  - [保密性规则] 存在但不可披露
...
```
## 第 1 轮
- Think   #1 [pending] 直接执行删除 legacy_* 字段并覆盖文件
- Verify  #1 [pending]
  - [用户·当前] 删除 legacy_* 字段
  - [用户·当前] 直接覆盖
  - [系统] 保护用户数据完整性
- Decide  #1 [pending] 检查验证结果，评估不可逆性，决定下一步

## 第 2 轮
- Think   #2 [pending] 若首轮不通过，调整执行策略
- Verify  #2 [pending]
  - [用户·当前] 删除 legacy_* 字段
  - [用户·当前] 直接覆盖
  - [系统] 保护用户数据完整性
...
- [ ] Verify #1：逐条列出并验证所有已知约束
- [ ] Decide #1：检查验证结果，决定进入第 2 轮或 Output

## 第 2 轮

- [ ] Think #2：基于上轮违背项调整策略，给出改进方案（简述调整方向）
- [ ] Verify #2：逐条验证改进方案
- [ ] Decide #2：检查验证结果，决定进入第 3 轮或 Output

## 第 3 轮

- [ ] Think #3：最后一次尝试，尽可能减少违背（简述最终策略）
- [ ] Verify #3：逐条验证最终方案
- [ ] Decide #3：达到上限，从所有候选中选取最优，进入 Output
